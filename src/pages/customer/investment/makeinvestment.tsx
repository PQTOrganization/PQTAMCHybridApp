...
const MakeInvestment = () => {
  ...
  const { getFunds, getFundsByAcctCategory, getFundsWithBanks } =
    useFundsService();
  const { getUserApplication } = useUserApplicationService();
  ...
  const [banksOfSelectedFund, setBanksOfSelectedFund] = useState<any>([]);
  const [bankDDList, setBankDDList] = useState<any>([]);
  ...
  useEffect(() => {
    ...
    getUserDetails().then((userDetails: any) => {
      ...
      getUserApplication(userDetails.userApplicationId).then((response) => {
        accountCategoryId = response.accountCategoryId;
      });
      ...
      return Promise.all([
        ...
        getUserApplication(userDetails.userApplicationId).then((response) => {
          accountCategoryId = response.accountCategoryId;
          getFundsByAcctCategory(accountCategoryId).then((funds) => {
            setFunds(funds);
  ...
  useEffect(() => {
    if (data?.fund) {
      ...
      let selectedBankList = banks.filter(
        (b: any) => b.fundId === data.fund.id
      );

      const newBanksList = selectedBankList.map((f: any) => {
        return { id: f.branchId, value: f.bankName };
      });

      setBanksOfSelectedFund(selectedBankList);
      setSelectedBank(newBanksList?.[0]);
      setBankDDList(newBanksList);
    }
  }, [data.fund]);

  useEffect(() => {
    if (data?.bank) {
      let bank = banksOfSelectedFund.find(
        (bank: any) => bank.branchId === data?.bank.id
      );
  ...
  const handleSubmit = () => {
    if (validateForm()) {
      if (parseInt(data.fund.id) in HighRiskFunds) {
        ...
        if (data.modeOfPayment.id === 4 || data.modeOfPayment.id === 6)
          navigate("/confirm-payment", {
            state: {
              data,
            },
            ...
            <GridDX container sx={{ width: "100%" }} rowSpacing={1}>
              ...
              <GridDX item xs={12}>
                <SelectListDX
                  ...
                  list={bankDDList}
                  value={{
                    id: selectedBank?.branchId,
                    value: selectedBank?.bankName,
                  }}
                  onChange={(e: any, v: any) => handleSelectChange(v, "bank")}
                  error={errors["bank"] ? true : undefined}
